...
void setup() {
  ...
  mcp.pinMode(PIN_ATS_CONT, INPUT_PULLUP);
  mcp.pinMode(PIN_ATS_CONF, INPUT_PULLUP);
  mcp.pinMode(PIN_ROOM_LIGHT, INPUT_PULLUP);
  mcp.pinMode(PIN_LIGFT_DEF, INPUT_PULLUP);
  mcp.pinMode(PIN_LIGHT_ON, INPUT_PULLUP);
  mcp.pinMode(PIN_PANTO, INPUT_PULLUP);
...
void keyboard_control(void) {
...
#ifdef N_MODE
  if (notch_brk == 9) {
    if (mcp.digitalRead(PIN_MC_DEC) != mc_DEC_latch) {
      if (!mcp.digitalRead(PIN_MC_DEC)) {
        Serial.println("Co");
      } else {
        Serial.println("N ");
  ...
  mc_DEC_latch = mcp.digitalRead(PIN_MC_DEC);
#endif
...
void read_Ats(void) {
  Ats_Cont = !mcp.digitalRead(PIN_ATS_CONT);
  ...
  Ats_Conf = !mcp.digitalRead(PIN_ATS_CONF);
...
void read_Panto(void) {
  Panto = !mcp.digitalRead(PIN_PANTO);
